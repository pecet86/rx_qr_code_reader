...
import static pl.pecet.rx_ean_reader.R.id.qr_code_fragment;
import static pl.pecet.rx_ean_reader.R.layout.rx_ean_reader_fragment;
import static pl.pecet.rx_ean_reader.R.string.rx_ean_reader_wrong_type;
...
public class RxEanCodeFragment extends BaseFragment {
    ...
    private MaybeEmitter<EanCode> emitter;
    private EanConfig config;

    public RxEanCodeFragment() {
        super(rx_ean_reader_fragment);
    }
        ...
        requireActivity()
                ...
                .addCallback(requireActivity(), new OnBackPressedCallback(true) {
                    ...
                    public void handleOnBackPressed() {
                        Fragment fragment = getChildFragmentManager().findFragmentById(qr_code_fragment);
    ...
    public void request(@NonNull EanConfig config, @NonNull MaybeEmitter<EanCode> emitter) {
        this.config = config;
    ...
    void initViewFragment(@NonNull ViewFragment fragment) {
        fragment.init(config, new ViewFragment.Listener() {
            ...
            @Override
            public void check(@NonNull EanCode qrCode) {
    ...
    private synchronized void toView(EanCode qrCode) {
        if (config.getType() != null && !Objects.equals(qrCode.getType(), config.getType())) {
            Toast.makeText(requireContext(), rx_ean_reader_wrong_type, LENGTH_SHORT).show();
            return;
        }
